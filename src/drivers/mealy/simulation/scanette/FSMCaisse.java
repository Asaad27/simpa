...
public class FSMCaisse implements Caisse {
    ...
    private boolean nextConnectionRelecture;

    public double THRESHOLD = 0.1;
    

    public FSMCaisse(String pathToProductFile) throws Scanette.ProductDBFailureException {
        try {
            produits = new ArticleDB();
            produits.init(pathToProductFile);
            achats = new HashMap<Article, Integer>();
            etat = ETAT_CAISSE.EN_ATTENTE;
            nextConnectionRelecture = true;
        }
        catch (IOException | FileFormatException e) {
            throw new Scanette.ProductDBFailureException();
        }
    }
