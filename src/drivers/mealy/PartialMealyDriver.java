...
public abstract class PartialMealyDriver extends Driver<String, String> {
    ...
    public final GenericInputSequence.GenericOutputSequence execute(GenericInputSequence in) {
        GenericInputSequence.Iterator it = in.inputIterator();
        while (it.hasNext()) {
     ...
     * {@link CompleteMealyDriver#getDefinedInputs()}
     *
     * @param input
     * @return
     */
     ...
     *             {@link LmConjecture#searchInitialState(MultiTrace)})
     */
                ...
                if (ce == null) {
                    ...
                    if (resetIsAllowed) {
                        ...
                        int conjectureBound = conjecture.getStateCount()
                                * conjecture.getInputSymbols().size() * 100 + 500;
...
//		}
        ...
        if (!answer.equals("auto")) {
            ...
            for (String i : answer.split(",")) {
                if (!conjecture.getInputSymbols().contains(i))
...
MultiTrace trace, int maxLength, RandomGenerator randomGenerator) {
        ...
        if (Options.getLogLevel().compareTo(Options.LogLevel.ALL) >= 0)
            LogManager.logInfo("Starting a random walk");
        int tried = 0;

        while (maxLength < 0 || tried < maxLength) {
            List<String> is = getDefinedInputs();
    ...
    public State searchCurrentState(Mealy automata, LmTrace trace,
        ...
        while (possibleCurrentState.size() > 1) {
            ...
            if (distinction == null || distinction.isEmpty()) {
                Iterator<State> it = possibleCurrentState.iterator();
                s1 = it.next();
                s2 = it.next();
    ...
    protected boolean searchConjectureError_implem(LmConjecture conj) {
        ...
        int nbtests = conj.getInputSymbols().size() * 10;
