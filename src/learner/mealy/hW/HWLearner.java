...
public class HWLearner extends Learner {
     ...
     * @throws CeExposedUnknownStateException if a new state is discovered while searching the initial
     *                                        state in conjecture.
     */
     ...
     *
     * @param newW The sequence to add in W. This is not allowed to be a prefix
     *             of an existing sequence in W
     * @param W    The set to extend
     */
    ...
    private GenericInputSequence proceedHException(GenericHNDException e) {
        ...
        if (e instanceof AdaptiveHomingSequenceChecker.AdaptiveHNDException) {
            AdaptiveHomingSequenceChecker.AdaptiveHNDException adaptiveE =
                    (AdaptiveHomingSequenceChecker.AdaptiveHNDException) e;
            adaptiveE.updateH();
            if (Options.getLogLevel() != LogLevel.LOW)
        ...
        if (options.addHInW.isEnabled()) {
            ...
            assert !options
                    .useAdaptiveW() : "not implemented : add h in W and adaptive w sequences are incompatible at this" +
                    " time";
    ...
    public void learn() {
        ...
        List<String> inputAlphabet = driver.getDefinedInputs();
        ...
        if (options.useAdaptiveW()) {
            ...
            if (options.addIInW()) {
                for (String input : dataManager.getInputAlphabet())
        ...
        boolean inputSymbolDiscovered = false;
        int nrOfStates = 1;
        ...
        do {
            ...
            try {
                try {
                    boolean continueLastLearning = stateDiscoveredInCe && !inputSymbolDiscovered;
                    inputSymbolDiscovered = false;
                    learn(W, h, inputAlphabet, continueLastLearning);
                    assert checkTraces();
            ...
            } catch (NewInputsFoundException e) {
                LogManager.logInfo("New inputs discovered: " + e.getDelta());
            ...
            // on driver.
            if (!inputSymbolDiscovered && !inconsistencyFound && options.searchCeInTrace.isEnabled()
                    && counterExampleTrace == null) {
...
/*
            ...
            if (!inputSymbolDiscovered && !inconsistencyFound && counterExampleTrace == null) {
            ...
            if (counterExampleTrace != null) {
                ...
                LogManager
                        .logInfo("geting smallest suffix in counter example which is not in W and not a prefix of a W" +
                                " element");
                if (driver instanceof TransparentMealyDriver
     ...
     *
     * @param states          the states used to know which characterization should be
     *                        extended.
     * @param trace           the trace in which suffixes are taken
     * @param firstPosInTrace an offset in trace. the suffix starting at
     ...
     *
     * @param aStart a reachable State giving the same answer than bStart to h but
     *               leading in a different state.
     * @param bStart a reachable State giving the same answer than aStart to h but
     *               leading in a different state.
     */
    private void proceedHNotHomingforConjecture(State aStart, State bStart) {
        ...
        while (existingWTried != null) {
            ...
            if (!conjecture.apply(w, aEnd).equals(conjecture.apply(w, bEnd))) {
                ...
                if (path != null) {
                    ...
                    if (Options.getLogLevel() != LogLevel.LOW) {
                        LogManager.logConsole(
                                "inconsistency found : h is not homming for conjecture. Moreover there is a path in " +
                                        "conjecture to raise an inconsistency of type one or two");
                    }
                    dataManager.apply(
        ...
        if (Options.getLogLevel() != LogLevel.LOW) {
            if (statesAreEquivalents)
                LogManager.logInfo("States " + aEnd + " and " + bEnd
                        + " have the same characterization in conjecture. We cannot say that h is not homming because" +
                        " conjecture is not minimal.");
            else
                LogManager.logInfo(
                        "conjecture is not connex and we don't have a path to try the other state if we apply h and a" +
     ...
     *
     * @param trace          a trace observed from real automaton
     * @param currentState   the state in conjecture from wich the trace was applied (or
     *                       {@code null} if unknown)
     * @param globalTracePos the position of first transition in global trace (without
    ...
    private boolean searchAndProceedCEInOneTrace(LmTrace trace,
        ...
        for (int i = 0; i < trace.size(); i++) {
            ...
            if (newCompatibles.isEmpty()) {
                if (!extendWfailed)
                    LogManager.logInfo(
                            "There is no state compatible with the trace. This is a W-ND but we don't know were to " +
                                    "add sequence in the distinction structure");
                newCompatibles = new HashSet<>(dataManager.getStates());
            }
    ...
    public void learn(
            DistinctionStruct<? extends GenericInputSequence, ? extends GenericOutputSequence> W,
            GenericInputSequence h, List<String> inputAlphabet, boolean continueLastLearning)
        ...
        do {
            ...
            if (!driver.isDefined(x)) {
                FullyKnownTrace trace = new FullyKnownTrace(lastKnownQ, new LmTrace(x,
                        OUTPUT_FOR_UNDEFINED_INPUT), lastKnownQ);
                //dataManager.apply(input);
                for (var wResponse : lastKnownQ.getWResponses().knownResponses()) {
                    if (wResponse.size() > 0 && wResponse.getInput(0).equals(x)
                            && !wResponse.getOutput(0).equals(OUTPUT_FOR_UNDEFINED_INPUT)) {
                        dataManager.apply(x);
                    }
                }
                ...
                assert dataManager.getCurrentState() == null : "we are trying to qualify this state, that should not " +
                        "be already done.";
     ...
     *
     * @param referenceFile the dot file containing reference automata
