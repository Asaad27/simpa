	...
	private Request abstractToConcrete(String input){
		...
		try {
			if (input.equals("REGISTER")){
				...
				if (lastResp != null && lastResp.getHeader("Proxy-Authenticate") != null){
					ProxyAuthenticate auth = (ProxyAuthenticate)lastResp.getHeader("Proxy-Authenticate");
					ProxyAuthorizationHeader auth_hdr = hdr_factory.createProxyAuthorizationHeader("Digest");
					auth_hdr.setAlgorithm("MD5");
					auth_hdr.setNonce(auth.getNonce());
					auth_hdr.setRealm(auth.getRealm());
					auth_hdr.setUsername("user1test");
