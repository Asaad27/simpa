	...
	public static void main(String[] args) {
		...
		try {
			...
			if (Options.RETEST == -1){
				...
				stats.setHeader("Test", "States", "Inputs", "Outputs", "Equalities", "NDVs", "ARL", "Requests", "Duration", "LessThan", "LessThanEquals", "Greater");
				...
				for (i=1; i<=Options.NBTEST; i++){
					...
					try {
						...
						if (!Options.STAT) System.out.print(" (" + record.get(record.size()-2) + ")\n");
					}finally {
						LogManager.end();
						LogManager.clear();
						System.gc();
