...
public class HTTPResponse {
	...
	public HTTPResponse(String response){
		...
		if (status[0].equals("HTTP/1.1")) this.version = Version.v11; else this.version = Version.v10;
		code = Integer.parseInt(status[1]);
		codeString = status[2];
		for (int i=3; i<status.length; i++) codeString += " " + status[i];		
	...
	public String toString(){
		...
		s.append("HTTP/" + (version==Version.v10?"1.0":"1.1") + " " + code + " " + codeString + "\n");
