	...
	public String toString(){
		...
		for(EFSMTransition t : conj.getTransitions()){
			...
			if (!conj.getLabelForTransition(t).getPredicates().isEmpty()){
				...
				transitions.append("            <label kind=\"guard\">" + Utils.joinAndClean(predicates, " | ").replace("&", "and").replace("|", "or").replace("=", "==") +"</label>\n");
			}
			...
			for(String s : conj.getLabelForTransition(t).getOutputFunctions()){
				if (s.indexOf("=>") == -1){
					...
					if (value[1].startsWith("\"Ndv")) transitions.append("            <label kind=\"assignment\">" + value[0] + " := random();</label>\n");
					else{
						try{
							Float.parseFloat(value[1].substring(1, value[1].length()-1));
							transitions.append("            <label kind=\"assignment\">" + value[0] + " := " + value[1].substring(1, value[1].length()-1).trim() + "</label>\n");
						}catch(NumberFormatException e){
							transitions.append("            <label kind=\"assignment\">" + value[0] + " := " + value[1].trim() + "</label>\n");
					...
					transitions.append("            <label kind=\"assignment\"> " + "if (" + Utils.filter(value1[0]).replace("&", "and").replace("|", "or") + "){ " + value2[0] + " := " + value2[1] + "; } </label>\n");
				}
			}
			...
			transitions.append("            <label kind=\"output\">"+ output +"</label>\n");
		...
		for(String key : symbols.keySet()){
			declarations.append("            " +symbols.get(key) + " " + Utils.capitalize(key) + ";\n");
		}
		declarations.append("        </declaration>\n");
