...
public class LiControlTable {
	...
	public LiControlTable(List<String> inputSymbols, Map<String, List<ArrayList<Parameter>>> defaultParamValues2) {
			...
			for(int i=0; i<ctr.getColumCount(); i++){
				for(int j=0; j<ctr.getSizeOfColumn(i); j++){
					for(int k=j+1; k<ctr.getSizeOfColumn(i); k++){
						if (!ctr.getItemInColumn(i, j).getOutputSymbol().equals(ctr.getItemInColumn(i, k).getOutputSymbol())){
							List<ArrayList<Parameter>> parameters = new ArrayList<ArrayList<Parameter>>();
							for(int l=0; l<ctr.getSizeOfColumn(i); l++){
								parameters.add((ArrayList<Parameter>)ctr.getItemInColumn(i, l).getParameters());								
							}
							NDF ndf = new NDF(ctr.getPIS().removeEmptyInput(), inputSymbols.get(i), parameters);
		...
		for(int i=0; i<inputSymbols.size(); i++){
			for(int z=0; z<allRows.size(); z++){
				for(int y=0; y<allRows.get(z).getSizeOfColumn(i); y++){
					List<Parameter> parametersA = allRows.get(z).getItemInColumn(i, y).getParameters();
					for(int x=0; x<allRows.size(); x++){
						if (x == z) continue;
						boolean found = false;
						for(int w=0; w<allRows.get(x).getSizeOfColumn(i); w++){
							List<Parameter> parametersB = allRows.get(x).getItemInColumn(i, w).getParameters();
						...
						if (!found){
							...
							for(int k=0; k<allRows.get(z).getSizeOfColumn(i); k++){
								boolean foundReal = true;
								for(int l=0; l<allRows.get(z).getItemInColumn(i, k).getParameters().size(); l++){
									if (!allRows.get(z).getItemInColumn(i, k).getParameter(l).equals(nbp.params.get(l))){
										foundReal = false;
										break;
									}
								}
								if (foundReal){
									for(int l=0; l<allRows.get(z).getItemInColumn(i, k).getParameters().size(); l++){
										nbp.setNdvIndex(l, allRows.get(z).getItemInColumn(i, k).getParameterNDVIndex(l));
									}
								}
							}
							return nbp;
	...
	public List<LiControlTableRow> getRowStartsWith(ParameterizedInputSequence pis){
		final List<LiControlTableRow> allRows = getAllRows();
		List<LiControlTableRow> ctrs = new ArrayList<LiControlTableRow>();
		for (LiControlTableRow ctr : allRows){
			if (ctr.getPIS().startsWith(pis)) ctrs.add(ctr);
		}
		return ctrs;
	}
	...
	public int getFromState(LiControlTableRow ctr) {
		...
		if (pis.sequence.isEmpty()) pis.addEmptyParameterizedInput();
		for(LiControlTableRow c : allRows){
			if (pis.isSame(c.getPIS())) return getToState(c);
		}
		return 0;
	}
	...
	public int getToState(LiControlTableRow ctr) {
		for(int i=0; i<S.size(); i++){
			if (ctr.isEquivalentTo(S.get(i))) return i;
		}		
		return 0;
	}
	...
	private void initialize(){
		...
		for (int i=0; i<inputSymbols.size(); i++){
			...
			seq.addParameterizedInput(new ParameterizedInput(inputSymbols.get(i)));
			E.add(seq);
		}
		...
		for (int i=0; i<inputSymbols.size(); i++){
			List<ArrayList<Parameter>> params = defaultParamValues.get(inputSymbols.get(i));
			if (params == null){
				throw new RuntimeException("Unable to find default parameter values for input symbol \""+inputSymbols.get(i)+"\"");
			}else{
				for(int l=0; l<params.size(); l++){
					pis = new ParameterizedInputSequence();
					pis.addParameterizedInput(inputSymbols.get(i), defaultParamValues.get(inputSymbols.get(i)).get(l));
