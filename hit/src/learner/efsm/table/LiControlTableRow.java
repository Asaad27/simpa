...
public class LiControlTableRow {
	...
	public LiControlTableRow(ParameterizedInputSequence pis, List<ParameterizedInputSequence> E) {
		this.pis = pis;
		this.E = E;
		this.equiv = -1;
	...
	public void addAtColumn(int iColumn, LiControlTableItem cti){
		...
		if (!exists) cols.get(iColumn).add(cti);		
	}
	...
	public boolean isEpsilon(){
		return ((pis.sequence.size() == 1) && (pis.getLastSymbol().equals(EFSM.EPSILON)));
	}
	...
	public boolean isEquivalentTo(LiControlTableRow aRow) {
		...
		for(int i=0; i<getColumCount(); i++){
			setISa.clear();
			setISb.clear();
			for(int j=0; j<getSizeOfColumn(i); j++) setISa.add(getItemInColumn(i, j).getOutputSymbol());
			for(int j=0; j<aRow.getSizeOfColumn(i); j++) setISb.add(aRow.getItemInColumn(i, j).getOutputSymbol());
			if (!setISa.equals(setISb)) return false; 
		}
		return true;
	}
}
