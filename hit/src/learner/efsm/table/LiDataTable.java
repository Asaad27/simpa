...
public class LiDataTable {
	...
	public LiDataTable(List<String> inputSymbols,Map<String, List<ArrayList<Parameter>>> defaultParamValues) {
	...
	public void addAtCorrespondingPlace(LiDataTableItem dti, ParameterizedInputSequence currentPis) {
		...
		if (prefix.sequence.isEmpty()) prefix.addEmptyParameterizedInput();
		...
		for(LiDataTableRow dtr : allRows){
			for(int j=0; j<dtr.getColumCount(); j++){
				if (!dtr.getColum(j).isEmpty()){
					...
					int ndvIndex = newNDVForItem(dtr.getColum(j), params, ndvList.size());
					if (ndvIndex != -1){
						...
						NDV aNdv = new NDV(pi.removeEmptyInput(), last.type, ndvIndex, ndvList.size());						
	...
	public void initialize(){
		...
		for (int i=0; i<inputSymbols.size(); i++){
			for(int l=0; l<defaultParamValues.get(inputSymbols.get(i)).size(); l++){
	...
	public int newNDVForItem(List<LiDataTableItem> item, List<Parameter> params, int index) {
						...
						if (!(item.get(i).getOutputParameters().get(k).value.equals(item.get(j).getOutputParameters().get(k).value))){
							...
							if (!exists){
								if (currentIndex == index){
									params.addAll(item.get(i).getInputParameters());
									params.add(item.get(i).getOutputParameters().get(k));
									return k;
								}else{
									oldParams.add((ArrayList<Parameter>)item.get(i).getInputParameters());
									currentIndex++;
								}
							}
						}
	...
	public Set<Parameter> getFixedOutputParametersFor(ParameterizedInputSequence pis) {
		...
		for(LiDataTableRow d : S){
			if (d.getPIS().isSame(pis)) { dtr = d; break; }
		}
		pis.addParameterizedInput(pi);
		pis.removeEmptyInput();
		for (LiDataTableItem dti : dtr.getColum(inputSymbols.indexOf(pi.getInputSymbol()))){
			boolean isEqual = true;
			for(int i=0; i<dti.getInputParameters().size(); i++){
				if (!dti.getInputParameters().get(i).equals(pi.getParameters().get(i))){
					isEqual = false;
					break;
				}
			}
			if (isEqual){
				NDV ndv = null;
				for(NDV n : ndvList) {
					if (n.getPIS().isSame(pis)) ndv = n;
				}
				for(int i=0; i<dti.getOutputParameters().size(); i++) {
					if (ndv == null || ndv.paramIndex != i){
						if (!values.contains(dti.getOutputParameters().get(i).value)){
