...
public class DataManager {
		...
		for (FullyQualifiedState q : Q.values()){
			for (FullyKnownTrace t : q.getVerifiedTrace()){
				...
				if (C.get(traceSize() - t.getTrace().size()) == t.getStart()) {
					LmTrace subtrace = getSubtrace(traceSize()
							- t.getTrace().size(), traceSize());
					LmTrace tTrace = t.getTrace();
					if (subtrace.getInputsProjection().equals(
							tTrace.getInputsProjection())) {
						if (subtrace.getOutputsProjection().equals(
								tTrace.getOutputsProjection())) {

							setC(traceSize(), t.getEnd());
						} else {
							C.set(traceSize(), t.getEnd());// if we use `setC`
															// here, we
															// implicitly add a
															// new transition
															// and get errors
							...
							throw new InconsistancyWithConjectureException(tTrace,subtrace);
						}
					}
				}
			}
	...
	public boolean addPartiallyKnownTrace(FullyQualifiedState start, LmTrace transition, LmTrace print){
		...
		assert (W.contains(print.getInputsProjection())||(W.size()==0&&print.size()==0));
