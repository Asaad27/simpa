...
public class Relation {	
	...
	public Relation(RelationType type, int a1, String name1, int a2, String name2){
		this.a1 = a1;
		this.a2 = a2;
		this.name1 = name1;
		this.name2 = name2;
							...
							for(int j=i+1; j<attrCount; j++){
								...
								if (inst.attribute(i).type() == inst.attribute(j).type()){ 
									if (inst.attribute(i).isNumeric()){
										if (inst.value(i) == inst.value(j)) r = new Relation(t, i, inst.attribute(i).name(), j, inst.attribute(j).name());
									}else{
										if (!inst.stringValue(i).equals(inst.stringValue(j))) r = new Relation(t, i, inst.attribute(i).name(), j, inst.attribute(j).name());
									}
								}
								if (r != null){
	...
	public boolean equals(Object to){
	    ...
	    if ( !(to instanceof Relation) ) return false;
