...
public class TreeNode {
	...
	private int nodeLevel() {
		if (lines[currLine].length() == 0)
			return -1;
		else {
			int l = 0;
			while (lines[currLine].substring(l * 4).startsWith("|   "))
				l++;
			return l;
		}
	}
	...
	private TreeNode readNode(int level) {
		...
		if (line.indexOf(": ") != -1)
			cond = line.substring(0, line.indexOf(": ")).trim();
		else
			cond = line.trim();
		if (cond.startsWith("rel"))
			cond = cond.substring(cond.indexOf("=") + 2);
		if (line.indexOf(": ") == -1)
			data = null;
		else
	...
	public void loadFromString(Classifier cl) {
		...
		while (!lines[currLine].isEmpty())
			currLine++;
		currLine++;
		if (currLine == 4) {
			data = lines[2].substring(lines[2].indexOf(": ") + 2,
					lines[2].indexOf("(", lines[2].indexOf(": ") + 2)).trim();
			condition = "(true)";
		} else {
			while (nodeLevel() == 0) {
	...
	private List<String> getPredicatesFor_Rec(String leaf, String c,
			List<String> pred) {
		if (leaf.equals(data))
			pred.add(c + "(" + condition + ")");
		else {
			if (condition != null) {
				for (TreeNode ntn : child) {
					ntn.getPredicatesFor_Rec(leaf, c + "(" + condition + ") "
							+ Options.SYMBOL_AND + " ", pred);
				}
			} else {
				for (TreeNode ntn : child) {
