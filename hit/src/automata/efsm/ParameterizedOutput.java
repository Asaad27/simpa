...
public class ParameterizedOutput implements Cloneable{
	...
	public ParameterizedOutput(String output, List<Parameter> paramaters){
		this.outputSymbol = output;
		this.parameters = (ArrayList<Parameter>)((ArrayList<Parameter>)paramaters).clone();
	}
	...
	public ParameterizedOutput clone(){
		return new ParameterizedOutput(outputSymbol, (ArrayList<Parameter>)((ArrayList<Parameter>)parameters).clone());
	}
	...
	public String toString(){
		if (isOmegaSymbol()) return Options.SYMBOL_OMEGA_UP;
		else{
			StringBuffer s = new StringBuffer(outputSymbol + "(");
			if (parameters.size()>0) s.append(parameters.get(0).value);
			for(int i=1; i<parameters.size(); i++) s.append(", " + parameters.get(i).value);
			s.append(")");
			return s.toString();
		}
	}
