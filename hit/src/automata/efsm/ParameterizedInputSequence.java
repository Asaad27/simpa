...
public class ParameterizedInputSequence implements Cloneable{
	...
	public boolean equals(ParameterizedInputSequence o){
		if (sequence.size() != o.sequence.size()) return false;
			...
			for(int i=0; i<sequence.size(); i++){
				if (!sequence.get(i).getInputSymbol().equals(o.sequence.get(i).getInputSymbol())) return false;
			}			
		}
		return true;
	}
	...
	public ParameterizedInputSequence removeEmptyInput(){
		if (sequence.get(0).isEpsilonSymbol()) sequence.remove(0);
		return this;
	}
	...
	public boolean startsWith(ParameterizedInputSequence pis){
		...
		if (pis.sequence.size()<=sequence.size()){
			for(int i=0; i<pis.sequence.size(); i++){
				if ((!pis.sequence.get(i).getInputSymbol().equals(sequence.get(i).getInputSymbol())) ||
					(!pis.sequence.get(i).getParamHash().equals(sequence.get(i).getParamHash()))) return false;
			}
			return true;
		}else return false;
	}
	...
	public String toString(){
		...
		for (ParameterizedInput pi : sequence){
			if (!pi.isEpsilonSymbol()) s.append(pi.toString());			
			else s.append(Options.SYMBOL_EPSILON);
		}
		return s.toString();
	}
	...
	public String getHash() {
		String hash = "";
		for(int i=0; i<sequence.size(); i++){
			hash += sequence.get(i).getInputSymbol() + sequence.get(i).getParamHash(); 
		}
		return hash;
	}
}
