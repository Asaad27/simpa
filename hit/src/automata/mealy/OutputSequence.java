...
public class OutputSequence implements Cloneable{
	...
	public OutputSequence removeOmegaOutput(){
		if (sequence.get(0).length()==0) sequence.remove(0);
		return this;
	}
		
	public boolean startsWith(OutputSequence os){
		if (os.sequence.isEmpty() || sequence.isEmpty()) return false;
		if (os.sequence.size()<=sequence.size()){
			for(int i=0; i<os.sequence.size(); i++){
				if ((!os.sequence.get(i).equals(sequence.get(i)))) return false;
			}
			return true;
		}else return false;
	}
	...
	public String toString(){
		...
		for (String input : sequence){
			if (input.length()>0) s.append(input.toString());			
			else s.append(Options.SYMBOL_OMEGA_LOW);
		}
		return s.toString();
	}
}