...
public class RandomEFSM extends EFSM implements Serializable {	
	...
	private void generateSymbols() {
		...
		for(int i=0; i<nbSym; i++){
			...
			arity.put("in"+i, Utils.randIntBetween(Options.MINPARAMETER, Options.MAXPARAMETER));
		}
		outputSymbols = new ArrayList<String>();
		nbSym = Utils.randIntBetween(Options.MININPUTSYM, Options.MAXINPUTSYM);
		for(int i=0; i<nbSym; i++){
			outputSymbols.add("out" + i);
			arity.put("out"+i, Utils.randIntBetween(Options.MINPARAMETER, Options.MAXPARAMETER));
		}
	...
	public static RandomEFSM deserialize(String filename){
		...
		LogManager.logStep(LogManager.STEPOTHER, "Loading RandomEFSM from " + f.getName());
		try {
	...
	private void createDefaultParameterValues() {
		...
		for(String symb : inputSymbols){
			...
			for (int i=0; i<arity.get(symb); i++){
				ll.add(new Parameter(String.valueOf(Options.DOMAINSIZE*10 + (i+1)), Types.NUMERIC));
			}
			l.add(ll);
			dpv.put(symb, l);
		}
	...
	private void createSimpleGuards() {
		for(EFSMTransition t : transitions){
			if (Utils.randBoolWithPercent(Options.SIMPLEGUARDPERCENT)){
				LogManager.logInfo("Creating EQUALSTOVALUE guard for state " + t.getFrom() + " with input " + t.getInput());
				ArrayList<Parameter> newParameters = t.randomizeGuard();
				simpleCount++;
				List<ArrayList<Parameter>> existingParameters = dpv.get(t.getInput());
				boolean alreadyExists = false;
				for (ArrayList<Parameter> existingParameter : existingParameters){
					boolean equals = true;
					for(int i=0; i<newParameters.size(); i++){
						if (!newParameters.get(i).value.equals(existingParameter.get(i).value)){
							equals = false;
							break;
						}
					}
				...
				if (!alreadyExists) existingParameters.add(newParameters);
			}
		}
	}
	
	private boolean isCleanForGenerateGuard(EFSMTransition t){
		return !t.getFrom().isInitial() && !t.getGuard().checkNdv && !t.generateNdv && !t.getFrom().equals(t.getTo());
	}
	
	private boolean isCleanForCheckingGuard(EFSMTransition t){
		return !t.getFrom().isInitial() && !t.getGuard().checkNdv && !t.generateNdv && (getTransitionTo(t.getFrom(), false).size() == 1);
	}	

	private void createNdvGuards() {
		int nbNdv = 0;
		for(EFSMTransition t : transitions){
			if (isCleanForGenerateGuard(t) && Utils.randBoolWithPercent(Options.NDVGUARDPERCENT*2)){
				State gen = goRandomStepsFrom(Utils.randIntBetween(Options.NDVMINTRANSTOCHECK, Options.NDVMAXTRANSTOCHECK)-1, t.getTo());
				EFSMTransition check = Utils.randIn(getTransitionFrom(gen, false));  
				if (isCleanForCheckingGuard(check)){
					LogManager.logInfo("State " + t.getFrom() + " generates a Ndv"+nbNdv);
					t.generateNdv(nbNdv);					
					LogManager.logInfo("State " + gen + " will check Ndv"+nbNdv);
	...
	private State goRandomStepsFrom(int nbSteps, State s) {
		...
		while (nbSteps>0){			
			for(EFSMTransition t : transitions){
				if (t.getFrom().equals(tmp) && !t.getTo().equals(tmp)) l.add(t);
			}
		...
		for(List<EFSMTransition> group : groups){
			if (group.size()>0){
				...
				for(int i=1; i<group.size(); i++){
					if (remainingOutputSymbols.contains(group.get(i).getOutput()))
						remainingOutputSymbols.remove(group.get(i).getOutput());
					else{
						String newOutputSymbol = Utils.randIn(remainingOutputSymbols);
	...
	private void fixUnreachableStates() {
		...
		while (unreachables.size()>0){
			...
			// List with the number of outgoing transition from state "from" for each input symbol
			List<Integer> nbTransWithInput = new ArrayList<Integer>();
			for(int i=0; i<inputSymbols.size(); i++) nbTransWithInput.add(0);
			for (EFSMTransition t : transitions){
			...
			for (int i=1; i<nbTransWithInput.size(); i++)
				if (nbTransWithInput.get(minIn) > nbTransWithInput.get(i)) minIn = i;

			String in = inputSymbols.get(minIn);

			// List with the number of outgoing transition from state "from" with the input symbol "in"
			List<Integer> nbTransWithOutput = new ArrayList<Integer>();
			for(int i=0; i<inputSymbols.size(); i++) nbTransWithOutput.add(0);
			for (EFSMTransition t : transitions){
			...
			for (int i=1; i<nbTransWithOutput.size(); i++)
				if (nbTransWithOutput.get(minOut) > nbTransWithOutput.get(i)) minOut = i;
			...
			if (nbTransWithOutput.get(minOut)>=2){
				...
				LogManager.logInfo("States " + s + " have been fixed adding transition to " + from + " to " + s);
				addTransition(new EFSMTransition(this, from, s, in, out, new GeneratedOutputFunction(arity.get(in), arity.get(out))));
				reachables.add(s);				
			}
	...
	private void fixDeadEndStates() {
		...
		for(State s : deadend){
			...
			if (to != null){
				...
				addTransition(new EFSMTransition(this, s, to, in, out, new GeneratedOutputFunction(arity.get(in), arity.get(out))));
	...
	private void classifyDeadEndStates(Set<State> deadend, Set<State> notdeadend) {
		...
		notdeadend.clear();
		for(EFSMTransition t : transitions){
			if (!t.getFrom().equals(t.getTo())) notdeadend.add(t.getFrom());			
		}
	...
	private void classifyStatesUnreachable(Set<State> reach, Set<State> unreach) {
		...
		do{
			...
			for(State s : newreachable){
				for (EFSMTransition t : transitions){
					if (t.getFrom().equals(s)) tmp.add(t.getTo());
				}
			}
			newreachable = tmp;
	...
	private void createTransitions() {
		...
		for(State s1 : states){
			...
			nbTrans = 0;
			for(State s2 : states){
				if (Utils.randBoolWithPercent(Options.TRANSITIONPERCENT/states.size()) && nbTrans<2){
					boolean added = false;
					nbTrans++;
					while (!added){
						in = Utils.randIn(inputSymbols);
						List<String> outExists = getOutputForThisInput(s1, in);
						if (outExists.size() == outputSymbols.size()) continue;
						do
							out = Utils.randIn(outputSymbols);
						while (previous.contains(in + "|" + out)); 
						addTransition(new EFSMTransition(this, s1, s2, in, out, new GeneratedOutputFunction(arity.get(in), arity.get(out))));
						previous.add(in + "|" + out);
						added = true;
					}
				}				
	...
	public TreeMap<String, List<String>> getDefaultParamNames() {
		...
		for(String symbol : arity.keySet()){
			...
			for(int i=0; i<arity.get(symbol); i++) names.add(symbol + "p" + i);
