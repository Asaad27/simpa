...
public class Dummy {
	...
	public static EFSM getAutomata() {
		...
		test.addTransition(new EFSMTransition(test, s2, s3, "B", "D", new IOutputFunction() {			
			@Override
			public List<Parameter> process(EFSM automata, List<Parameter> inputParameters) {
				List<Parameter> p = new ArrayList<Parameter>();
				p.add(new Parameter(String.valueOf(Integer.parseInt(inputParameters.get(0).value)*2), Types.NUMERIC));
				return p;
			}
		}));
		test.addTransition(new EFSMTransition(test, s3, s1, "A", "D", new IOutputFunction() {			
			@Override
			public List<Parameter> process(EFSM automata, List<Parameter> inputParameters) {
